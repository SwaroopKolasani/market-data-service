...
def test_create_poll_job_success(client: TestClient):
    ...
    request_data = {"symbols": ["AAPL", "MSFT"], "interval": 60, "provider": "yahoo_finance"}

    with patch("app.services.price_service.PriceService.create_polling_job") as mock_create_job:
        mock_create_job.return_value = {"job_id": "poll_12345", "status": "accepted", "config": request_data}
...
def test_create_poll_job_invalid_interval(client: TestClient):
    ...
    request_data = {"symbols": ["AAPL"], "interval": 30, "provider": "yahoo_finance"}  # Too short
